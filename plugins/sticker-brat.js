...
if (!text) return conn.sendMessage(m.chat, { text: '🌸 Por favor, responde a un mensaje o escribe un texto para crear el pétalo.' }, { quoted: m })
await m.react('🕒')
const buffer = await fetchSticker(text)
const stiker = await sticker(buffer, false, texto1, texto2)
if (!stiker) throw new Error('🌷 No se pudo generar el pétalo.')
...
case 'emojimix': {
if (!args[0]) return m.reply(`🌸 Combina 2 emojis para crear un pétalo nuevo.\n> Ejemplo: *${usedPrefix + command}* 👻+👀`)
...
if (!res.results || res.results.length === 0) throw new Error('🌷 No se encontraron pétalos para esos emojis.')
...
if (!textFinal) return conn.reply(m.chat, `🌸 Escribe un texto para crear el pétalo.`, m)
...
if (frase.length > 30) return await m.react('✖️'), conn.reply(m.chat, `🌷 El texto no puede tener más de 30 pétalos de letras.`, m)
await m.react('🕒')
...
case 'take': case 'wm': {
if (!m.quoted) return m.reply(`🌸 Responde a un pétalo con *${usedPrefix + command}* y el nuevo nombre.\n> Ejemplo: *${usedPrefix + command}* Jardín•Sakurita`)
await m.react('🕒')
const stickerData = await m.quoted.download()
if (!stickerData) return await m.react('✖️'), m.reply('🌷 No se pudo descargar el pétalo.')
...
await m.react('✖️')
conn.sendMessage(m.chat, { text: `🥀 Ocurrió un problema.\n> Usa *${usedPrefix}report* para reportarlo.\n\n${e.message}` }, { quoted: m })
}}
